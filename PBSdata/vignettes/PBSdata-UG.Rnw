...
\tab \pkg{PBSdata} contains data objects used primarily by the R packages \pkg{PBStools} for illustrating function examples and \pkg{PBSmapx} for drawing various coastline and boundary shapefiles. The data herein either exist in the public domain or are non-proprietary and open for public use.
