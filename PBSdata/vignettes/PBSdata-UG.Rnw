...
\tab \pkg{PBSdata} contains data objects used primarily by the R package \pkg{PBStools} for illustrative purposes. The data herein either exist in the public domain or are non-proprietary and open for public use.

\tab A useful set of data (regional boundaries, key codes, example data), a GUI interface to \code{PBSmapping}, and handy utility functions evolved over time (2007--2012) in one R package called \pkg{PBSfishery}, which can be found on Google's Project Hosting site: \url{http://code.google.com/p/pbs-fishery/}. In April 2012, we decided to split this package into three separate libraries -- \code{PBSdata}, \code{PBSmapx}, and \code{PBStools} -- for distribution on CRAN.
...
\tab The \pkg{PBSdata} help files provide examples to illustrate the data objects; however, these examples have been disabled using the \code{dontrun\{\}} enclosure because we want to keep \pkg{PBSdata} from depending on other packages. Many of the examples require \pkg{PBSmapping}, and a few will require \pkg{PBStools}. In turn, \pkg{PBStools} depends on \pkg{PBSmapping}, \pkg{PBSmodelling}, \pkg{PBSdata}, and \pkg{RODBC}.
...
\pbsfig{orfhistory}{Barplot showing the various historical records of total rockfish (t) other than Pacific 
  Ocean Perch. In years where sources overlap, the catch reconstruction either sums the catches or takes
...
%species
\pbsfig{species}{Data frame of species described primarily by \citet{Hart:1973} codes. The information resides in a data table 
	called \code{C\_Species} in the relational database \code{PacHarvest}, and is supplemented from species codes in \code{GFBIO}.}{5.9}
